---
name: update-harness
description: AGENTS.md・AGENTS.md ルールファイル・メモリを実装に追従して自動更新するセルフメンテナンススキル。3日に1回の定期実行を想定。
---

DevNext Harness（AGENTS.md・AGENTS.md rules・メモリ）を現在の実装に追従させて更新してください。
...
- `AGENTS.md` — Codex 専用設定（@rules/ 参照）
- `.Codex/rules/*.md` — 各ルールファイル
- `C:/Users/harne/.Codex/projects/H--Codex-DevNext/memory/MEMORY.md` — メモリインデックス
...
| スキルが増えていないか | `AGENTS.md` や `rules/` への追記要否 |
...
`C:/Users/harne/.Codex/projects/H--Codex-DevNext/memory/` の各ファイルを確認し、
...
```bash
# AGENTS.md はforce-trackされているので個別に追加する
git add .Codex/rules/ .Codex/skills/ AGENTS.md
git commit -m "chore: harness を実装に追従して自動更新"
```

**注意**: `.Codex/rules/` と `.Codex/skills/` は `.gitignore` の例外として追跡済み。
`AGENTS.md` は `git add -f` でforce-trackされているため、通常の `git add` で追加できる。
