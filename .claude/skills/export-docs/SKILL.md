...
```bash
# docx バリデーション（Windows では cp932 エンコードエラーが出るが無視してよい）
python scripts/office/validate.py output.docx
```

**Windows での validate.py の cp932 エラーについて:**
`'cp932' codec can't decode byte...` というエラーは、Python が UTF-8 エンコードの XML を cp932 で読もうとする Windows 環境固有の問題。docx ファイルの実態（段落数・テーブル数）が正しく出力されていれば無視してよい。
