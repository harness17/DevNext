...
| 21 | ExcelItemEntity | Excel サンプル商品 | アプリ |
| 22 | ExcelItemEntityHistory | ExcelItemEntity 履歴 | アプリ |
...
### 3.21 ExcelItemEntity

Excel サンプル機能の商品テーブル。ClosedXML を使ったエクスポート・インポートのサンプルデータを格納する。

| カラム名 | 型 | NULL | 制約 | 説明 |
|---------|---|------|------|------|
| `Id` | bigint | NOT NULL | PK, IDENTITY | 商品 ID |
...
### 3.22 ExcelItemEntityHistory

ExcelItemEntity の変更履歴テーブル。インポート・更新の都度旧データをコピーして保持する。

ExcelItemEntity と同じカラム構成に `HistoryId`（bigint, PK, IDENTITY）を追加。
